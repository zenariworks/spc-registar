...
A deactivated Clanstvo must not auto-resolve to that parish, must be
...
class ResolveTenantTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.tenant2 = Zakupac.objects.get(schema_name="test_tenant_res2")
    ...
    def test_active_membership_resolves_and_sets_session(self):
        Clanstvo.objects.create(
            korisnik=self.user, parohija=self.tenant2, uloga=Uloga.PREGLED
        )
