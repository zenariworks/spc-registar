...
(function () {
  ...
  const dataEl = document.getElementById('form-errors-data');
  if (!dataEl) return;
  ...
  function decorateField(name, messages) {
    ...
    const row = input.closest('.info-row');
    if (!row) return null;
  ...
  Object.keys(errorsByField).forEach(function (name) {
    const messages = errorsByField[name];
    if (!messages || !messages.length) return;
    const input = decorateField(name, messages);
