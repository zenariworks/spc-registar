...
# MARRIAGE
# --------------------------------------------------------------------
def assert_spouse_pair(
    zenik_gender: str,
    zenik_birthdate: date,
    nevesta_gender: str,
    nevesta_birthdate: date,
    datum_vencanja: date,
):
    ...
    assert (
        nevesta_gender == "Ж"
    ), f"невеста мора бити женска, добила: {nevesta_gender!r}"
    assert_adult(zenik_birthdate, datum_vencanja, "женик")
    assert_adult(nevesta_birthdate, datum_vencanja, "невеста")
    diff = abs(
        age(zenik_birthdate, datum_vencanja) - age(nevesta_birthdate, datum_vencanja)
    )
...
def assert_krstenje(
    dete_birthdate: date,
    datum_krstenja: date,
    otac_gender: str | None = None,
    majka_gender: str | None = None,
):
    """Child age <5 at baptism, datum after birth, parents have valid genders."""
    assert (
        datum_krstenja >= dete_birthdate
