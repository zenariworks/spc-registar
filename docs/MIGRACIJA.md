...
тенант (`tenant_cukarica`). Цео ток траје око 4 минута.

```bash
# 1) Поставите шему и креирајте подразумеваног тенанта.
#    Креира public шему + tenant_cukarica шему са свим registar табелама.
python manage.py migrate_schemas

# 2) Учитајте DBF фајлове у staging табеле унутар тенант шеме.
python manage.py tenant_command load_dbf --schema=tenant_cukarica \
    --src_zip /путања/до/crkva.zip

# 3) Сејте lookup табеле (народности, вероисповести, занимања, епархије, славе).
python manage.py tenant_command unos_narodnosti      --schema=tenant_cukarica
python manage.py tenant_command unos_veroispovesti   --schema=tenant_cukarica
python manage.py tenant_command unos_zanimanja       --schema=tenant_cukarica
python manage.py tenant_command unos_eparhija        --schema=tenant_cukarica
python manage.py tenant_command unos_slava           --schema=tenant_cukarica

# 4) Покрените миграције за сваки модел.
python manage.py tenant_command migracija_svestenika        --schema=tenant_cukarica
python manage.py tenant_command migracija_ukucana_parohijana --schema=tenant_cukarica
python manage.py tenant_command migracija_krstenja          --schema=tenant_cukarica
python manage.py tenant_command migracija_vencanja          --schema=tenant_cukarica
```
...
```bash
python manage.py tenant_command shell --schema=tenant_cukarica -c "
...
**TENANT** шеме (`tenant_cukarica`, `tenant_vracar`, …) свака садржи:
...
2. Поставља `search_path` на `tenant_<name>, public`.
...
Затим пунте податке у нову шему истим током (`tenant_command ... --schema=tenant_vracar`).
